...
use dumb_cgi::{Body, EmptyResponse, FullResponse, Request};
...
fn load_library() -> Result<HashMap<String, HashMap<u16, FontData>>, String> {
    ...
    let lib: HashMap<String, HashMap<u16, FontData>> = match serde_json::from_reader(&f) {
        Ok(x) => x,
        Err(e) => {
...
*/
fn field_name_from_content_disposition(val: &str) -> Option<&str> {
...
fn list_response() -> ! {
    ...
    let mut list_map: HashMap<String, Vec<u16>> = HashMap::with_capacity(lib.len());
...
fn render_from_server_font(
    ...
    let family = fonts
        .get(font_name)
        .ok_or(format!("No font data matching \"{}\".", font_name))?;

    let font = family.get(&size).ok_or(format!(
    ...
    write_f(image, font, &mut r).map_err(|e| format!("Error writing text image: {}", &e))?;
...
fn render_response(req: &dumb_cgi::Request) -> ! {
    ...
    let body_parts = match req.body() {
        Body::Multipart(v) => v,
        _ => {
            error_response(400, "Request is not multipart/form-data.");
        }
    };
    ...
    for part in body_parts.iter() {
        ...
        if let Some(val) = part.headers.get("content-disposition") {
            match field_name_from_content_disposition(val) {
                Some("font") => {
                    let font = String::from_utf8_lossy(&part.body).trim().to_string();
                    font_name = Some(font);
                }
                Some("font_file") => {
                ...
                Some("font_source") => {
                    if part.body == "user".as_bytes() {
                        user_supplied_font = true;
                    }
                }
                Some("size") => match std::str::from_utf8(&part.body) {
                ...
                Some("invert") => {
                    if "true" == String::from_utf8_lossy(&part.body) {
                        invert = true;
                    }
                }
    ...
    let response = if user_supplied_font {
        let font_file =
            font_file.unwrap_or_else(|| error_response(400, "Missing \"font_file\" falue."));

        match render_from_user_font(font_file, size, &image, invert) {
            Ok(r) => r,
            Err(ref s) => {
                error_response(500, s);
            }
        }
    } else {
        let font_name = font_name.unwrap_or_else(|| error_response(400, "Missing \"font\" value."));
